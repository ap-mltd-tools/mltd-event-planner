...
export default function Dashboard() {
  ...
  const tab: Tab = isTab(tabParam) ? tabParam : "home";
  ...
  return (
    <div className="app">
      ...
      <nav className="tabs">
        <button className="tab" onClick={() => changeTab("home")}>
          {t("dashboard.home")}
        </button>

        <button className="tab" onClick={() => changeTab("calculate")}>
          {t("dashboard.stockAndSpendPlayCalculator")}
        </button>
      </nav>

      <main className="content">{screens[tab]}</main>
    </div>
  );
}
